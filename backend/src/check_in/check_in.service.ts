...
export class CheckInService {
   ...
   * Support: itemCode (WRISTBAND/BIB), ticketCode (NONE), wristbandCode (legacy)
   */
  async checkIn(checkInDto: CheckInDto) {
    ...
    if (checkInDto.itemCode) {
      ...
      throw new BadRequestException('Must provide itemCode, ticketCode');
    }
  }
  ...
  async checkInByTicketCode(ticketCode: string) {
    return this.dataSource.transaction(async (manager) => {
      // 1️⃣ Cari tiket berdasarkan ticketCode
